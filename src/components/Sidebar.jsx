...
export default function Sidebar({ onClose, isCollapsed, onCollapseToggle }) {
  ...
  return (
    <aside
      className={`${isCollapsed ? "w-20" : "w-64"} bg-surface-container-low flex flex-col border-r border-outline-variant/10 h-full transition-[width] duration-300 ease-in-out`}
    >
      <div
        className={`p-6 flex flex-col gap-1 ${isCollapsed ? "items-center px-2" : ""} overflow-y-auto no-scrollbar flex-1`}
      >
        {/* Logo + Close */}
        <div
          className={`flex items-center ${isCollapsed ? "justify-center" : "justify-between"} mb-6 shrink-0 overflow-hidden`}
        >
          <div className="flex items-center gap-3">
            ...
            >
              <h1 className="text-on-background font-bold text-sm tracking-tight leading-none whitespace-nowrap">
                BrokerPortal
              </h1>
              <p className="text-secondary text-[10px] uppercase tracking-widest mt-1 whitespace-nowrap">
        ...
        {/* Navigation */}
        <nav
          className={`flex flex-col gap-1 mt-2 text-left ${isCollapsed ? "items-center w-full" : ""}`}
        >
          {navItems.map((group) => (
            <div
              key={group.section}
              className={`mb-4 ${isCollapsed ? "w-full flex flex-col items-center" : ""}`}
            >
              {!isCollapsed && (
                <p className="text-[10px] uppercase tracking-widest font-bold text-secondary/60 px-3 mb-2">
                  {group.section}
                </p>
              )}
      ...
      {/* User Profile */}
      <div
        className={`mt-auto shrink-0 border-t border-outline-variant/10 ${isCollapsed ? "p-4 flex flex-col items-center gap-2" : "p-6 flex flex-col gap-3"}`}
      >
        <div
          className={`bg-surface-container rounded-xl border border-outline-variant/10 ${isCollapsed ? "p-2" : "p-4 w-full"}`}
        >
          <div
            className={`flex items-center ${isCollapsed ? "justify-center" : "gap-3"}`}
          >
            <div className="w-10 h-10 rounded-full bg-surface-container-highest flex items-center justify-center text-primary font-bold text-sm shrink-0">
              {initials}
            </div>
            {!isCollapsed && (
