...
const SEARCH_RESULTS = [
  {
    group: "Quick Actions",
    items: [
      { icon: "account_balance", text: "Deposit Funds", link: "/deposit" },
      {
        icon: "currency_exchange",
        text: "Transfer Internal",
        link: "/accounts",
      },
    ],
  },
  {
    group: "Navigation",
    items: [
      { icon: "security", text: "Security Settings", link: "/security" },
      { icon: "manage_accounts", text: "Profile Details", link: "/settings" },
      {
        icon: "receipt_long",
            ...
            <div className="absolute top-full left-0 mt-2 w-[calc(100vw-32px)] sm:w-full max-w-[400px] bg-white rounded-xl shadow-2xl border border-outline-variant/15 overflow-hidden z-[100] animate-fade-in">
              <div className="p-3 border-b border-outline-variant/10 bg-background/50">
                <p className="text-[10px] uppercase tracking-widest text-secondary font-bold">
                  Try searching for "Deposit", "Security", or "Accounts"
                </p>
              </div>
              <div className="max-h-[60vh] overflow-y-auto no-scrollbar py-2">
                {SEARCH_RESULTS.map((group, idx) => (
                  <div key={idx} className="mb-2 last:mb-0">
                    <p className="px-4 py-1.5 text-[10px] uppercase tracking-widest text-secondary font-bold bg-background/50">
                      {group.group}
                    </p>
                    {group.items
                      .filter((item) =>
                      ...
                      .map((item, i) => (
                        ...
                        >
                          <span className="material-symbols-outlined text-[18px] text-secondary group-hover/item:text-primary transition-colors">
                            {item.icon}
                          </span>
                          <span className="text-sm font-medium text-on-surface">
                            {item.text}
                          </span>
                          <span className="material-symbols-outlined text-[16px] text-secondary/30 ml-auto group-hover/item:text-primary transition-colors">
                            arrow_forward
                          </span>
                        </button>
                      ))}
        ...
        <div className="relative" ref={balanceRef}>
          ...
          >
            ...
            <span className="font-bold text-on-surface text-sm">
              {hideBalance ? "******" : "10,045.15"}{" "}
              <span className="text-secondary text-xs">USD</span>
            </span>
          </button>
          ...
          {showBalanceDropdown && (
            <div className="absolute top-full right-0 mt-3 w-[280px] sm:w-[320px] bg-white rounded-xl shadow-2xl border border-outline-variant/15 overflow-hidden z-[100] animate-fade-in origin-top-right">
              ...
              <div className="p-4 flex items-center justify-between border-b border-outline-variant/10 bg-dark/5">
                ...
                >
                  <span
                    className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${hideBalance ? "translate-x-4" : "translate-x-1"}`}
                  />
                </button>
              </div>
              ...
              <div className="p-4 border-b border-outline-variant/5 hover:bg-surface-container/30 transition-colors group">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-surface-container hover:bg-surface-container-high text-on-surface font-medium py-2 rounded-lg text-sm transition-colors border border-outline-variant/10"
                  >
                    Transfer
                  </button>
                  <button
              ...
              <div className="p-4 border-b border-outline-variant/5 hover:bg-surface-container/30 transition-colors group">
                <div className="mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="bg-surface-container text-secondary text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                      8
                    </span>
                  </div>
                  <p className="text-xs text-secondary/60 font-mono mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity absolute">
                    View balances
                  </p>
                </div>
                <button
                  ...
                  className="w-full bg-surface-container hover:bg-surface-container-high text-on-surface font-medium py-2 rounded-lg text-sm transition-colors border border-outline-variant/10"
                >
                  Go to Wallet
                </button>
              </div>
        ...
        >
          ...
          >
            <span
              ...
              style={showNotifs ? { fontVariationSettings: "'FILL' 1" } : {}}
            >
              notifications
            </span>
            ...
            <div className="absolute top-full right-0 mt-3 w-[320px] sm:w-[360px] bg-white rounded-xl shadow-2xl border border-outline-variant/15 overflow-hidden z-[100] animate-fade-in origin-top-right">
              ...
              <div className="max-h-[400px] overflow-y-auto no-scrollbar">
                {NOTIFICATIONS.map((notif) => (
                  <div
                    key={notif.id}
                    className="p-4 border-b border-outline-variant/5 hover:bg-surface-container/50 transition-colors cursor-pointer group flex gap-3 items-start"
                  >
                    <div
                      className={`mt-0.5 shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${notif.type === "success" ? "bg-emerald-50 text-emerald-600" : notif.type === "warning" ? "bg-amber-50 text-amber-600" : "bg-primary/10 text-primary"}`}
                    >
                      <span
